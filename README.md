...
An AI-powered personal finance analytics platform that ingests bank statement PDFs, automatically categorizes and analyzes spending, detects unusual transactions, forecasts next month's spend, answers natural-language questions about your spending, and gets smarter the more you use it.

> **Status: local-first / self-hosted.** This project previously ran on Google Cloud Run; it now runs entirely locally (backend + frontend on your own machine, database on Supabase's free tier). There's no hosted live demo at the moment — see "Running Locally" below to spin it up yourself. This was a deliberate choice while the project takes on heavier, more experimental features, not a sign anything is broken.
...
- **Answer natural-language questions about your spending** via a chat interface, grounded in your own computed analytics rather than free-floating LLM guesses — "why did my spending jump in June?" triggers real tool calls against your actual trends, anomalies, and budget data, not a hallucinated answer
...
- Groq API (OpenAI-compatible, free tier) with tool-use/function-calling for the LLM chat insights feature
- Runs locally; a `Dockerfile` is included if you want to self-host or deploy it yourself
...
- Runs locally via Vite's dev server; can be deployed to any static host (Vercel, Netlify, etc.)
...
The chat insights feature uses a tool-use loop: the LLM never sees raw transaction data, only pre-computed summaries (trends, anomalies, forecasts, budget outlook) it requests via defined tools, so answers stay grounded in real numbers rather than the model's own guesses.
...
- **The chat feature uses tool-calling, not context-stuffing.** Rather than dumping all transaction data into a prompt, the LLM requests specific pre-computed summaries via defined tools (get_anomalies, get_budget_outlook, etc.) — this keeps answers grounded in real analytics output and scales to questions that weren't anticipated in advance.
...
│   │   ├── routers/             # API endpoints, including chat.py
...
│   │   ├── llm/                 # Tool-use loop + tool definitions for chat insights
│   │   ├── models/              # SQLAlchemy models
│   │   └── services/            # Shared business logic
│   ├── Dockerfile                # Included for optional self-hosting/deployment
│   └── requirements.txt
├── frontend/
│   ├── src/
│   │   ├── components/          # Dashboard, Upload, Login, charts, ChatWidget
│   │   ├── context/              # Auth state
│   │   ├── api/                  # Backend API client
│   │   └── utils/
│   └── package.json
└── render.yaml                   # Alternate deployment config (Render), unused by default
```
...
cp .env.example .env   # fill in your own Supabase + database + Groq values
python init_db.py
...
| `GROQ_API_KEY` | Groq API key (free tier) for the chat insights feature |
...
If you're testing OAuth (Google sign-in) locally, make sure `http://localhost:5173/**` is added under Supabase's **Authentication → URL Configuration → Redirect URLs**, alongside your production URL if you have one.

---

## Status

Actively developed, running locally. Supported statement formats: SBI "Relationship Summary" and "Statement of Account" layouts, including password-protected PDFs.

**Recently added:** LLM-powered chat insights (tool-use grounded in real analytics, via Groq's free tier).

**In progress:** currency formatting in chat responses, auto-generated monthly natural-language recap.
